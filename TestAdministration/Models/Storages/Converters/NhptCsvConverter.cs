...
/// <see cref="NhptCsvRecord"/> objects.
/// </summary>
public class NhptCsvConverter(
    NormInterpretationConverter normInterpretationConverter
)
{
    public Test FromRecord(NhptCsvRecord record)
    {
        var notes = ParseNotes(record.Notes, NhptCsvRecord.NoteNames);
    ...
    public NhptCsvRecord ToRecord(Test test) => new()
    {
        ...
        DominantPracticeNormInterpretation = normInterpretationConverter.Convert(test.Sections[0].Trials[0].SdScore),
        DominantFirstNormInterpretation = normInterpretationConverter.Convert(test.Sections[0].Trials[1].SdScore),
        DominantSecondNormInterpretation = normInterpretationConverter.Convert(test.Sections[0].Trials[2].SdScore),
        DominantThirdNormInterpretation = normInterpretationConverter.Convert(test.Sections[0].Trials[3].SdScore),
        DominantAverageNormInterpretation = normInterpretationConverter.Convert(test.Sections[0].AverageSdScore),
        NonDominantPracticeNormInterpretation = normInterpretationConverter.Convert(test.Sections[1].Trials[0].SdScore),
        NonDominantFirstNormInterpretation = normInterpretationConverter.Convert(test.Sections[1].Trials[1].SdScore),
        NonDominantSecondNormInterpretation = normInterpretationConverter.Convert(test.Sections[1].Trials[2].SdScore),
        NonDominantThirdNormInterpretation = normInterpretationConverter.Convert(test.Sections[1].Trials[3].SdScore),
